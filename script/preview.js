document.addEventListener("DOMContentLoaded", function () {
    ...
    if (book) {
        ...
        if (book.available) {
            document.getElementById("availability").classList.add('available-preview');
            document.getElementById("availability").classList.remove('borrowed-preview');
        }
        else {
            document.getElementById("availability").classList.remove('available-preview');
            document.getElementById("availability").classList.add('borrowed-preview');
        }
    ...
    borrowBtn.addEventListener('click', function() {
        ...
        if (borrowedIndex === -1) {
            ...
            // If the book is in favourites, update it
            if (favIndex !== -1) {
                // Update availability in favourites list
                favouriteBooks[favIndex].available = false; 
            }
        } else {
            // Return the book
            ...
            if (favIndex !== -1) {
                // Update availability in favourites list
                favouriteBooks[favIndex].available = true; 
            }
        }
        ...
        localStorage.setItem('favourite', JSON.stringify(favouriteBooks)); // Update favourites
