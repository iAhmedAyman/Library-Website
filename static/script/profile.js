...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (updateProfileBtn) {
      console.log("Update profile button found, adding event listener");
...
function updateProfile() {
    ...
    const firstName = inputs[0].value || user.username.split(' ')[0];
    const lastName = inputs[2].value || user.username.split(' ')[1];
    const email = inputs[1].value || user.email;
...
function showMessage(message, type) {
    ...
    // Insert after the update profile button
    const updateProfileBtn = document.querySelector('.update-profile');
    if (updateProfileBtn && updateProfileBtn.parentNode) {
      updateProfileBtn.parentNode.insertBefore(messageContainer, updateProfileBtn.nextSibling);
    } else {
      // Fallback - add to the form
      const form = document.getElementById('profile-form');
      if (form) {
